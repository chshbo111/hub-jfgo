...
class DialogueSystem:
    ...
    def need_repeat(self, query):
        need_repeat_sentences = ["你说啥？", "我没听清", "请重复一遍"]
        for sentence in need_repeat_sentences:
    ...
    def run(self, query, memory):
        ...
        '''
        memory["query"] = query
        if self.need_repeat(query):
            memory["response"] = memory["response"] or "请告诉我们您的需求"
            return memory
...
if __name__ == '__main__':
    ...
    memory = {"available_nodes":["scenario-买衣服_node1","scenario-看电影_node1"], "response": None} #用户状态
    while True:
        query = input("请输入：")
        # query = "你好"
