...
def main(config):
    ...
    for epoch in range(config["epoch"]):
        ...
        for index, batch_data in enumerate(train_data):
            ...
            # loss = nn.CrossEntropyLoss(ignore_index=-100)(output, labels.view(-1))
            loss = nn.CrossEntropyLoss(ignore_index=-100)(output.view(-1, output.shape[-1]), labels.view(-1))
            loss.backward()
            optimizer.step()
